...
def run_algorithm_by_name(
    algorithm_name, testcase_file_name, time_limit=0.1, average_time_limit=0.04
):
    try:
...
def run_all_algorithms(testcase_file_name, time_limit=0.1, average_time_limit=0.04):
    algorithm_folder = "./algorithms"
    for filename in os.listdir(algorithm_folder):
        if (
            filename.endswith(".py")
            and filename != "__init__.py"
            and filename != "algorithm_template.py"
        ):
